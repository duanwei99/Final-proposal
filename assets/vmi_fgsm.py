...
class PyFAT:
    ...
    def generate(self, im_a, im_v, n):
        ...
        mask_np = cv2.resize(cv2.imread(osp.join('../assets', 'mask.png')), (112, 112)) / 255
        ...
        for iters in tqdm(range(self.num_iters)):
            ...
            # get adv feature
            adv_feat_ir50 = self.model1.forward(adv_images)
            adv_feat_ir100 = self.model2.forward(adv_images)
            ...
            g = g * decay_factor + (grad + v) / (grad + v).abs().mean(dim=[1, 2, 3], keepdim=True)
            sum_grad = g

            # update v_{t+1}
            v = 0
            for _ in range(N):
                neighbor_image = adv_images.detach() + torch.randn_like(adv_images).uniform_(-eps * beta, eps * beta)
                neighbor_image.requires_grad = True
                neighbor_image = neighbor_image.to(self.device)

                # get adv feature
                nei_feat_ir50 = self.model1.forward(neighbor_image)
                nei_feat_ir100 = self.model2.forward(neighbor_image)

                # caculate loss and backward
                target = torch.tensor([1]).to(self.device)
                loss_ir50 = loss_func(nei_feat_ir50, vic_feat_ir50, target)
                loss_ir100 = loss_func(nei_feat_ir100, vic_feat_ir100, target)
                loss = loss_ir50 + loss_ir100
                # loss = loss_ir50
                loss.backward(retain_graph=True)
                v = v + neighbor_image.grad.data
            v = v / N - grad
